...
function initWhiteStorm() {
  ...
  WORLD.man_0_0 = new WHS.ConvexModel({
...
WORLD.man_0_0.addTo(WORLD, 'wait').then(function() {
  ...
  for (var i=0; i<=columns; i++) {
    for (var j=0; j<=rows; j++) {
      ...
      if (i>0 || j>0) {
        ...
        WORLD['man_'+i+'_'+j].position.set( firstR + (i*dist), 0, firstC + (j*dist) )
        WORLD['man_'+i+'_'+j].scale.set(20, 20, 20);
        WORLD['man_'+i+'_'+j].addTo(WORLD, 'wait');
  ...
  WORLD.light = new WHS.DirectionalLight({
